...
- 你通过加载 feasibility_report SKILL 获取所有Worker的模板定义
...
### 步骤12：Leader执行Word渲染（P0级，不可跳过）
- Integrator提交final（完整可研报告.md）后，Leader执行以下渲染步骤：
  1. 调用Skill工具加载 render_script SKILL（注意：是 render_script，不是 report_integration）
  2. render_script SKILL的正文就是完整的Python渲染脚本代码，直接将其全部内容写入 /tmp/render_feasibility.py
...
2. 禁止自行读取原始材料文件内容（步骤5汇总和步骤12渲染除外）
3. 禁止自行撰写报告章节内容

...
MD_H3_RE = re.compile(r'^###\s+(.+)$')
MD_HR_RE = re.compile(r'^---+\s*$')
...
def parse_blocks(text):
    ...
    while i < len(lines):
        ...
        if m:
            if cur_table: blocks.append({'type': 'table', 'title': cur_title, 'rows': cur_table}); cur_table, cur_title = [], None
            blocks.append({'type': 'heading1', 'text': m.group(1)}); i += 1; continue
        m = MD_H2_RE.match(line)
        if m:
            if cur_table: blocks.append({'type': 'table', 'title': cur_title, 'rows': cur_table}); cur_table, cur_title = [], None
            blocks.append({'type': 'heading2', 'text': m.group(1)}); i += 1; continue
        m = MD_H3_RE.match(line)
        if m:
            if cur_table: blocks.append({'type': 'table', 'title': cur_title, 'rows': cur_table}); cur_table, cur_title = [], None
            blocks.append({'type': 'heading3', 'text': m.group(1)}); i += 1; continue
...
def render(md_f, out_f, cn='企业名称', pn='项目名称'):
    ...
    # 过滤：找到第一个"第X章"标题，跳过其之前的所有块（封面信息由渲染脚本自动生成，不应出现在正文中）
    first_chapter_idx = None
    for idx, b in enumerate(blocks):
        if b['type'] == 'heading1' and re.match(r'^第[一二三四五六七八九十\d]+章', b['text']):
            first_chapter_idx = idx
            break
    if first_chapter_idx is not None and first_chapter_idx > 0:
        blocks = blocks[first_chapter_idx:]
    # 提取目录项（仅提取正文章节标题，不含第一章之前的内容）
    toc_items = []
    for b in blocks:
        if b['type'] == 'heading1':
            toc_items.append({'level': 1, 'text': b['text']})
        elif b['type'] == 'heading2':
            toc_items.append({'level': 2, 'text': b['text']})
    ...
    p3 = doc.add_paragraph(); p3.alignment = WD_ALIGN_PARAGRAPH.CENTER; set_font(p3.add_run(f'编制单位：{cn}'), '仿宋', Pt(15))
    ...
    for item in toc_items:
        if item['level'] == 1:
            p = doc.add_paragraph()
            set_font(p.add_run(item['text']), '黑体', Pt(12), True)
            p.paragraph_format.space_before = Pt(6)
            p.paragraph_format.space_after = Pt(3)
        else:
            p = doc.add_paragraph()
            set_font(p.add_run('    ' + item['text']), '仿宋', Pt(12), False)
